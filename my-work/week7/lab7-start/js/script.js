...
let theSituation = graphGroup.selectAll(".datapoint").data(data,function(d){
  return d.key;
});
...
function removeRec(){
  ...
  xAxis.tickFormat(d=>{return data.filter(dd=>dd.key==d)[0].name;});
  xAxisGroup.transition().duration(2000).call(xAxis).selectAll("text").attr("font-size", 18);
  ...
  theSituation.transition().duration(2000).attr("transform", function(d, i){
      return "translate("+ xScale(d.key)+ "," + (h - padding) + ")"
    });
...
function reverseData(){
  removeAndAddDatapoints(data.length, data.length);
  removeRec();
  addRec();
}
document.getElementById("buttonF").addEventListener("click", reverseData);
